...
import { parseApiToken, parseUrl, twirlLoader } from '../utils/misc'
...
export class JUnitUploadCommandModule implements CommandModule<unknown, JUnitArgs> {
	...
	handler = async (args: Arguments<JUnitArgs>) => {
		...
		await uploadTestCases(args, results, api)

		console.log(`Uploaded ${results.length} test cases`)
	}
}

const validateAndPrintMissingAttachments = (
	args: Arguments<JUnitArgs>,
...
const validateAndPrintMissingTestCases = (args: Arguments<JUnitArgs>, missing: JUnitTestCase[]) => {
