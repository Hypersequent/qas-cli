...
const getFixturePath = (fileType: (typeof fileTypes)[number], fixtureName: string): string => {
	...
	describe(`Uploading ${fileType.name} files`, () => {
		describe('Argument parsing', () => {
			test('Passing correct Run URL pattern should result in success', async () => {
				const patterns = [
					`${fileType.command} --run-url ${runURL} ${getFixturePath(fileType, 'matching-tcases')}`,
					`${fileType.command} -r ${runURL}/ ${getFixturePath(fileType, 'matching-tcases')}`,
					`${fileType.command} -r ${runURL}/tcase/1 ${getFixturePath(fileType, 'matching-tcases')}`,
				]
			...
			test('Passing incorrect Run URL pattern should result in failure', async () => {
				const patterns = [
					`${fileType.command} -r ${qasHost}/projects/${projectCode}/runs/${runId} ${getFixturePath(fileType, 'matching-tcases')}`,
					`${fileType.command} -r ${runURL}abc/tcase/1 ${getFixturePath(fileType, 'matching-tcases')}`,
				]
